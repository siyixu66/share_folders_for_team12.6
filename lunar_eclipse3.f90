SUBROUTINE compute_angle(DJM0,DJM,ASUN,RE,AM,CLIGHT,ANGLE_EOM,ANGLE_EO,ANGLE_MO,E2O,M2O)
        ...
        INTEGER :: B                              !目标天体编号
        INTEGER :: C                              !中心天体编号
        REAL *8::dt0,dt,dt_em2,dt_em1 
        REAL *8,DIMENSION(3)::E2S                 !地日距离
        REAL *8,DIMENSION(3)::E2O                 !地心与影锥距离
        REAL *8,DIMENSION(3)::E2M,E2M_               !地月距离
        ...
        E2M_=PO(1:3)
        dt=0
        dt0=NORM2(E2M_)/CLIGHT/86400.0
        DO WHILE(ABS(dt0-dt)>light_time_diff)
            dt0=dt
            CALL PLEPH(DJM0+DJM-dt0,B,C,PO)
            E2M_=PO(1:3)
            dt=NORM2(E2M_)/CLIGHT/86400.0
        END DO                                  !相切时刻月球相对于地球的位置
        dt_em2=dt

        CALL PLEPH(DJM0+DJM-dt_em2,B,C,PO)!
        E2M=PO(1:3)
        !光行时迭代
        dt=0!光行时，单位为天
        dt0=NORM2(E2M)/CLIGHT/86400.0 !
        DO WHILE(ABS(dt0-dt)>light_time_diff)
            dt0=dt
            CALL PLEPH(DJM0+DJM-dt_em2-dt0,B,C,PO)!    光传播到地球时月球相对于地球的位置
            E2M=PO(1:3)
            dt=NORM2(E2M)/CLIGHT/86400.0 !光行时，单位为天
        END DO
        dt_em1=dt
        !太阳发出光时刻太阳相对于地球的位置
        B=11
        CALL PLEPH(DJM0+DJM-dt_em1-dt_em2,B,C,PO)
        E2S=PO(1:3)
        dt=0
        dt0=NORM2(E2S)/CLIGHT/86400.0
        DO WHILE(ABS(dt0-dt)>light_time_diff)
            dt0=dt
            CALL PLEPH(DJM0+DJM-dt_em2-dt_em1-dt0,B,C,PO)!    光发射时，太阳相对于地球的位置
            E2S=PO(1:3)
...
PROGRAM MOON
          ...
          INTEGER::YEAR1=2025                     !结束的年
          INTEGER::MONTH1=12                         !结束的月
          INTEGER::DAY1=31                          !结束的日
